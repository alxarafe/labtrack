...
class Ordenes extends MY_Controller
{
    public function __construct()
    {
        ...
        $this->data['title'] = 'Gestión de expedientes';
        $this->data['description'] = 'Gestión de expedientes';
        $this->data['keywords'] = '';

        $this->data['css'][] = 'ordenes';
    ...
    public function index($orden = null, $id_centro = null, $id_familia = null, $id_proceso = null, $id_secuencia = null, $id_movimiento = null)
    {
        ...
        if (isset($_POST['guardar']) || isset($_POST['repetir'])) {
            ...
            if (isset($id_movimiento)) {
                $movimiento = $this->protesis_model->get_movimiento($id_movimiento);
                $user_id = isset($_POST['user']) ? $_POST['user'] : $this->user_id;
                $theuser = $this->auth_model->get_user_by_id($user_id);
                if (!isset($theuser)) {
                    $message = "El usuario $user_id no existe";
                    $user_id = $movimiento['id_operador'];
                }
                $this->protesis_model->save_data(
                    'movimientos',
                    array('id' => $id_movimiento),
                    array(
                        ...
                        'supervisado' => $movimiento['supervisado'], //$_POST['duracion']==$duracion?$this->user_id:0,
                        'notas' => '"' . $_POST['notas'] . '"',
                    )
                );
                redirect('/ordenes/index/' . $orden . (isset($message) ? "?message=$message" : ''));
            } else {
                $id_movimiento = $this->protesis_model->save_data(
                    ...
                    array(
                        'id_orden' => $orden,
                        'id_operador' => isset($_POST['operador']) ? $_POST['operador'] : $this->user_id,
                        'id_centrocosto' => $id_centro, //$_POST['centrocosto'],
                        'id_familia' => $id_familia,
                        'id_proceso' => $id_proceso,
                        'id_secuencia' => $id_secuencia,
                        ...
                        'duracion' => $_POST['duracion'],
                        'repetido' => isset($_POST['repetido']) ? (isset($_POST['fallointerno']) ? 2 : 1) : 0,
                        'supervisado' => 0, //$_POST['duracion']==$duracion?$this->user_id:0,
                        'notas' => '"' . $_POST['notas'] . '"',
                    )
                );
            }
    ...
    public function editar($orden = null)
    {
        if (!isset($orden)) {
            ...
            $page = 'altaorden';
            $_orden = $this->protesis_model->get_orden($orden);
            $this->data['alta'] = !$_orden;
            $this->data['orden'] = $orden;
            $this->data['nombre'] = isset($_orden) ? $_orden['nombre'] : '';
        }
        ...
        if (isset($_POST['guardar'])) {
            $id_movimiento = $this->protesis_model->save_data(
                ...
                array(
                    ...
                    'duracion' => $_POST['duracion'],
                    'repetido' => isset($_POST['repetido']) ? (isset($_POST['fallointerno']) ? 2 : 1) : 0,
                    'supervisado' => 0, //$_POST['duracion']==$this->data['secuencia']['duracion']?$this->user_id:0,
                    'notas' => '"' . $_POST['notas'] . '"',
                )
            );
            redirect("/ordenes/repetir/$id_centro/$id_familia/$id_proceso/$id_secuencia/$id_movimiento");
